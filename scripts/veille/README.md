...
Chaîne automatique hebdomadaire : **Judilibre** (arrêts Cass. civ. 3e + com., publiés au Bulletin, ajoutés à la base sur les 8 derniers jours) → **filtre baux commerciaux** (mots-clés, déterministe) → **scoring de pertinence** (Claude API, note 0-10 + angle d'article) → **digest email** (Resend).
...
## 2. Clé API Anthropic — ~5 min

1. **https://console.anthropic.com** → API Keys → Create Key.
2. Coût réel : ~1 appel/semaine sur quelques textes courts → **quelques centimes par mois**.
...
| `ANTHROPIC_API_KEY` | Clé Anthropic (étape 2) |
...
```bash
PISTE_CLIENT_ID=… PISTE_CLIENT_SECRET=… ANTHROPIC_API_KEY=… VEILLE_DRY_RUN=1 \
  node --experimental-strip-types scripts/veille/digest.ts
...
- Coût total : 0 € (Judilibre + Actions gratuits) + centimes Anthropic.
